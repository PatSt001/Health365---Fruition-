...
async function fetchAndApply(request) {
  ...
  if (url.pathname.startsWith("/app") && url.pathname.endsWith("js")) {
    ...
    /*
     * Diagnostic change:
     * Use a normal browser User-Agent when fetching the
     * Notion page instead of forwarding Google's crawler UA.
     */
    response = await fetch(url.toString(), {
      body: request.body,
      headers: {
  ...
  element(element) {
    element.append(
      ...
      window.history.pushState = function(state) {
        const dest = new URL(
          location.protocol + location.host + arguments[2]
        );
